<script setup>
import { computed } from 'vue';
import { Link, usePage } from '@inertiajs/vue3';
import { useI18n } from 'vue-i18n';

const { locale } = useI18n({ useScope: 'global' });
const page = usePage();
const isSuperAdmin = computed(() => page.props.auth?.user?.is_super_admin || false);

const props = defineProps({
    show: {
        type: Boolean,
        default: false,
    },
    // Kura atļauja trūkst - tiek parādīta modālajā logā caurspīdīguma labā
    requiredPermission: {
        type: String,
        default: null,
    },
});

const emit = defineEmits(['close']);

const lv = {
    title: 'Piekļuve liegta',
    subtitle: 'Jums nav atļaujas veikt šo darbību.',
    detail: 'Jūsu administratora konts šobrīd neietver šo atļauju. Ja domājat, ka tas ir kļūda, lūdzu sazinieties ar Super Administratoru.',
    permission: 'Nepieciešamā atļauja',
    goBack: 'Atpakaļ',
    goDashboard: 'Uz paneli',
    contactSuperAdmin: 'Sazināties ar Super Adminu',
    superAdminNote: 'Jūs esat Super Admins — šī darbība varētu būt bloķēta citā veidā.',
};
const en = {
    title: 'Access Denied',
    subtitle: 'You do not have permission to perform this action.',
    detail: 'Your administrator account does not currently include this permission. If you believe this is a mistake, please contact the Super Administrator.',
    permission: 'Required permission',
    goBack: 'Go Back',
    goDashboard: 'Go to Dashboard',
    contactSuperAdmin: 'Contact Super Admin',
    superAdminNote: 'You are a Super Admin — this action might be blocked in another way.',
};

const tx = computed(() => locale.value === 'lv' ? lv : en);
</script>

<template>
    <Transition name="unauth-fade">
        <div v-if="show" class="unauth-overlay" role="dialog" aria-modal="true">

            <!-- Fona aizmiglošanas slānis -->
            <div class="unauth-backdrop" @click="emit('close')"></div>

            <!-- Modāla panelis -->
            <div class="unauth-panel">

                <!-- Augšējais mirdzošais stienis -->
                <div class="unauth-glow-bar"></div>

                <!-- Brendings -->
                <div class="unauth-brand">
                    <img src="/img/RoltonsLV_Icon.png" alt="RalphMania Icon" class="brand-icon">
                    <img src="/img/name_logo.png" alt="RalphMania" class="brand-name">
                </div>

                <!-- Slēdzenes ikona -->
                <div class="lock-ring">
                    <div class="lock-inner">
                        <i class="fas fa-lock"></i>
                    </div>
                </div>

                <!-- Teksta saturs -->
                <h1 class="unauth-title">{{ tx.title }}</h1>
                <p class="unauth-subtitle">{{ tx.subtitle }}</p>
                <p class="unauth-detail">{{ tx.detail }}</p>

                <!-- Atļaujas mikroshēma -->
                <div v-if="requiredPermission" class="perm-chip">
                    <i class="fas fa-key"></i>
                    <span class="perm-label">{{ tx.permission }}:</span>
                    <code class="perm-code">{{ requiredPermission }}</code>
                </div>

                <!-- Galvenā administratora piezīme -->
                <div v-if="isSuperAdmin" class="superadmin-note">
                    <i class="fas fa-crown"></i>
                    {{ tx.superAdminNote }}
                </div>

                <!-- Darbības -->
                <div class="unauth-actions">
                    <button @click="$router.back ? $router.back() : emit('close')" class="btn-back">
                        <i class="fas fa-arrow-left"></i>
                        {{ tx.goBack }}
                    </button>
                    <Link href="/admin/dashboard" class="btn-dashboard">
                        <i class="fas fa-tachometer-alt"></i>
                        {{ tx.goDashboard }}
                    </Link>
                </div>

                <!-- Dekoratīvie stūra elementi -->
                <div class="corner corner-tl"></div>
                <div class="corner corner-br"></div>
            </div>

        </div>
    </Transition>
</template>

<style scoped>
/* Pārklājums */
.unauth-overlay {
    position: fixed;
    inset: 0;
    z-index: 9999;
    display: flex;
    align-items: center;
    justify-content: center;
    padding: 1rem;
}

.unauth-backdrop {
    position: absolute;
    inset: 0;
    background: rgba(2, 6, 23, 0.82);
    backdrop-filter: blur(6px);
    -webkit-backdrop-filter: blur(6px);
}

/* Panelis */
.unauth-panel {
    position: relative;
    width: 100%;
    max-width: 540px;
    background: #0f172a;
    border: 1px solid rgba(220, 38, 38, 0.35);
    border-radius: 1.25rem;
    padding: 2.5rem 2.5rem 2rem;
    text-align: center;
    box-shadow:
        0 0 0 1px rgba(220,38,38,0.1),
        0 25px 60px rgba(0,0,0,0.6),
        0 0 80px rgba(220,38,38,0.08);
    overflow: hidden;
}

/* Mirdzošais stienis augšpusē */
.unauth-glow-bar {
    position: absolute;
    top: 0; left: 0; right: 0;
    height: 3px;
    background: linear-gradient(90deg, transparent, #dc2626, #ef4444, #dc2626, transparent);
    border-radius: 1.25rem 1.25rem 0 0;
}

/* Stūra dekorācijas */
.corner {
    position: absolute;
    width: 2rem;
    height: 2rem;
    border-color: rgba(220,38,38,0.3);
    border-style: solid;
}
.corner-tl { top: 1rem; left: 1rem; border-width: 2px 0 0 2px; border-radius: 4px 0 0 0; }
.corner-br { bottom: 1rem; right: 1rem; border-width: 0 2px 2px 0; border-radius: 0 0 4px 0; }

/* Brendings */
.unauth-brand {
    display: flex;
    align-items: center;
    justify-content: center;
    gap: 0.875rem;
    margin-bottom: 1.75rem;
    opacity: 0.9;
}

.brand-icon {
    width: 2.5rem;
    height: 2.5rem;
    object-fit: contain;
    filter: drop-shadow(0 0 6px rgba(220,38,38,0.4));
}

.brand-name {
    height: 1.5rem;
    width: auto;
    object-fit: contain;
    /* nosaukuma logotipam ir melns fons - padariet to redzamu uz tumša paneļa */
    filter: brightness(0) invert(1) drop-shadow(0 0 4px rgba(220,38,38,0.3));
}

/* Slēdzenes ikona */
.lock-ring {
    width: 5rem;
    height: 5rem;
    margin: 0 auto 1.5rem;
    border-radius: 50%;
    background: rgba(220,38,38,0.1);
    border: 2px solid rgba(220,38,38,0.3);
    display: flex;
    align-items: center;
    justify-content: center;
    animation: pulse-ring 2.5s ease-in-out infinite;
}

.lock-inner {
    width: 3.25rem;
    height: 3.25rem;
    border-radius: 50%;
    background: rgba(220,38,38,0.15);
    display: flex;
    align-items: center;
    justify-content: center;
}

.lock-inner i {
    font-size: 1.375rem;
    color: #ef4444;
}

@keyframes pulse-ring {
    0%, 100% { box-shadow: 0 0 0 0 rgba(220,38,38,0.3); }
    50%       { box-shadow: 0 0 0 8px rgba(220,38,38,0); }
}

/* Teksts */
.unauth-title {
    font-size: 1.75rem;
    font-weight: 800;
    color: #f1f5f9;
    margin: 0 0 0.5rem;
    letter-spacing: -0.02em;
}

.unauth-subtitle {
    font-size: 1rem;
    color: #94a3b8;
    margin: 0 0 0.875rem;
    font-weight: 500;
}

.unauth-detail {
    font-size: 0.85rem;
    color: #64748b;
    margin: 0 0 1.5rem;
    line-height: 1.6;
    max-width: 400px;
    margin-left: auto;
    margin-right: auto;
}

/* Atļaujas mikroshēma */
.perm-chip {
    display: inline-flex;
    align-items: center;
    gap: 0.5rem;
    padding: 0.5rem 1rem;
    background: rgba(239,68,68,0.1);
    border: 1px solid rgba(239,68,68,0.25);
    border-radius: 0.5rem;
    margin-bottom: 1.25rem;
    font-size: 0.8rem;
}

.perm-chip i { color: #fca5a5; }
.perm-label  { color: #94a3b8; }
.perm-code   {
    font-family: 'Courier New', monospace;
    color: #fca5a5;
    font-weight: 700;
    background: rgba(239,68,68,0.15);
    padding: 0.125rem 0.375rem;
    border-radius: 0.25rem;
}

/* Galvenā administrtora piezīme */
.superadmin-note {
    display: flex;
    align-items: center;
    justify-content: center;
    gap: 0.5rem;
    padding: 0.625rem 1rem;
    background: rgba(251,191,36,0.1);
    border: 1px solid rgba(251,191,36,0.25);
    border-radius: 0.5rem;
    color: #fcd34d;
    font-size: 0.8rem;
    margin-bottom: 1.25rem;
}

/* Darbības */
.unauth-actions {
    display: flex;
    gap: 0.875rem;
    justify-content: center;
    margin-top: 0.5rem;
    flex-wrap: wrap;
}

.btn-back,
.btn-dashboard {
    display: inline-flex;
    align-items: center;
    gap: 0.5rem;
    padding: 0.7rem 1.5rem;
    border-radius: 0.625rem;
    font-size: 0.875rem;
    font-weight: 700;
    cursor: pointer;
    text-decoration: none;
    transition: all 0.2s;
    border: none;
}

.btn-back {
    background: rgba(255,255,255,0.06);
    color: #cbd5e1;
    border: 1px solid rgba(255,255,255,0.1);
}

.btn-back:hover {
    background: rgba(255,255,255,0.1);
    color: white;
}

.btn-dashboard {
    background: linear-gradient(135deg, #dc2626, #b91c1c);
    color: white;
    box-shadow: 0 4px 14px rgba(220,38,38,0.35);
}

.btn-dashboard:hover {
    transform: translateY(-1px);
    box-shadow: 0 6px 20px rgba(220,38,38,0.45);
}

/* Pāreja */
.unauth-fade-enter-active,
.unauth-fade-leave-active {
    transition: opacity 0.25s ease;
}
.unauth-fade-enter-active .unauth-panel,
.unauth-fade-leave-active .unauth-panel {
    transition: transform 0.25s ease, opacity 0.25s ease;
}
.unauth-fade-enter-from,
.unauth-fade-leave-to {
    opacity: 0;
}
.unauth-fade-enter-from .unauth-panel,
.unauth-fade-leave-to .unauth-panel {
    transform: scale(0.94) translateY(12px);
    opacity: 0;
}

/* Responsivitāte */
@media (max-width: 600px) {
    .unauth-panel { padding: 2rem 1.5rem 1.5rem; }
    .unauth-title { font-size: 1.375rem; }
    .unauth-actions { flex-direction: column; }
    .btn-back, .btn-dashboard { width: 100%; justify-content: center; }
}
</style>
